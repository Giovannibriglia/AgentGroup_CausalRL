...
from scripts.algorithms.DQN import ClassDQN, ReplayMemory


class SoftmaxAnnealingQAgent:
    def __init__(self, dict_alg_parameters, dict_env_parameters, predefined_q_table=None):
        self.rows = dict_env_parameters['rows']
        self.cols = dict_env_parameters['cols']
        self.action_space_size = dict_env_parameters['action_space_size']
        self.temperature = 1.0
        self.n_episodes = dict_env_parameters['n_episodes']
        self.alg = dict_env_parameters['alg']

        self.lr = dict_alg_parameters['LEARNING_RATE']
        self.gamma = dict_alg_parameters['GAMMA']

        self.exp_proba = dict_alg_parameters['START_EXPLORATION_PROBA']
        self.min_exp_proba = dict_alg_parameters['MIN_EXPLORATION_PROBA']
        self.exp_game_percent = dict_alg_parameters['EXPLORATION_GAME_PERCENT']
        self.EXPLORATION_DECREASING_DECAY = -np.log(self.min_exp_proba) / (self.exp_game_percent* self.n_episodes)

        self.if_deep = True if 'DQN' in self.alg else False

        if self.if_deep:
            self.batch_size = dict_alg_parameters['BATCH_SIZE']
            self.tau = dict_alg_parameters['TAU']
            self.replay_memory_capacity = dict_alg_parameters['REPLAY_MEMORY_CAPACITY']
...
class ThompsonSamplingQAgent:
    def __init__(self, dict_alg_parameters, dict_env_parameters, predefined_alpha_beta=None):
        self.rows = dict_env_parameters['rows']
        self.cols = dict_env_parameters['cols']
        self.action_space_size = dict_env_parameters['action_space_size']
        self.alg = dict_env_parameters['alg']
...
class EpsilonGreedyQAgent:
    ...
    def __init__(self, dict_alg_parameters, dict_env_parameters, predefined_q_table=None):
        self.rows = dict_env_parameters['rows']
        self.cols = dict_env_parameters['cols']
        self.n_agent_actions = dict_env_parameters['action_space_size']
        self.n_episodes = dict_env_parameters['n_episodes']
        self.alg = dict_env_parameters['alg']
